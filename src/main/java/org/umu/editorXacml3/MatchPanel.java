 ...
 *
 * Description:*//**
 * This panel is used for setting up a Match element.
 * 
 * @author Alberto Jim?nez L?zaro,Pablo Galera Morcillo
 * 
 * @version 1.3
 ***************************************************************************/
public class MatchPanel extends ElementPanel {
	...
	private void jbInit() throws Exception {
		...
		while (subelementos.hasMoreElements()) {
			...
			if (panelActual != null) {
				...
				if (elem instanceof Elemento3AttributeSelector || elem instanceof ElementoAttributeDesignatorAbstract) {
					miborde2 = new TitledBorder(new EtchedBorder(), "Choice");
					panelActual = new ConmutadorPanel(aux);
				} else {
					miborde2 = new TitledBorder(new EtchedBorder(), "<" + elem.getTipo() + ">");
				}
