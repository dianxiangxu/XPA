...
public class ConmutadorPanel
  ...
  private void jbInit() throws Exception {
    ...
    jrbDesignator.setBounds(new Rectangle(210, 20, 130, 20));
    ...
    if (elemento instanceof ElementoAttributeDesignatorAbstract) {
      jrbDesignator.setSelected(true);
      panelActual = new Panel3AttributeDesignator(node);
    }
    else if (elemento instanceof Elemento3AttributeSelector) {
      jrbSelector.setSelected(true);
      panelActual = new AttributeSelectorPanel(node);
    }
    else {
      panelActual = new ElementPanel(node);
    }
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() == jrbSelector) {
      ...
      ElementoXACML elem=(ElementoXACML)((DefaultMutableTreeNode)node.getParent()).getUserObject();
    ...
    elemento = ElementoXACMLFactoryImpl.getInstance().obtenerElementoXACML(
      ...
      elemento.setEmpty(true);
    node.setUserObject(elemento);
