...
public class MiRenderer
  ...
  public MiRenderer() {
    ...
    try{
      ...
      cara = createImageIcon("org/umu/icons/cara.gif");
      rule = createImageIcon("org/umu/icons/target.gif");
      verde = createImageIcon("org/umu/icons/verde.gif");
      nube = createImageIcon("org/umu/icons/nube.gif");
  ...
  protected boolean esTarget(Object value) {
    ...
    if(!(node.getUserObject() instanceof ElementoXACML)) return false;
    ElementoXACML st=(ElementoXACML)node.getUserObject();
    if(st.getTipo().equals(Elemento3Target.TIPO_TARGET)) return true;
    return false;
  }
